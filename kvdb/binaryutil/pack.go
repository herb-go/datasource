...
func getLengthBytes(data []byte) []byte {
	...
	if l < maxPackLengthNext2Byte {
		...
		DataOrder.PutUint16(result[1:], uint16(l))
		return result
	}
	if l < maxPackLengthNext4Byte {
		result = make([]byte, 5)
		result[0] = PackLengthCodeNext4Byte
		DataOrder.PutUint32(result[1:], uint32(l))
		return result
	}
	result = make([]byte, 9)
	result[0] = PackLengthCodeNext8Byte
	DataOrder.PutUint64(result[1:], uint64(l))
	return result
}
...
func readLength(r io.Reader) (length int64, err error) {
	...
	case PackLengthCodeNext2Byte:
		...
		length = int64(DataOrder.Uint16(buf))
		return
	...
	case PackLengthCodeNext4Byte:
		...
		length = int64(DataOrder.Uint32(buf))
		return
